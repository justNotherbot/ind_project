...
def get_random_num_pair():
    have_gcd = random.choice([True, False])
    gcd = 1
    if have_gcd:
        gcd = random.randint(2, 10**2)
...
def get_new():
    a, b = get_random_num_pair()
    c, d = get_random_num_pair()

    token = str(a) + "_" + str(b) + "_" + str(c) + "_" + str(d)
    in_str = str(a)+"\n"+str(b)+"\n"+str(c)+"\n"+str(d)+"\n"
    return token, in_str
